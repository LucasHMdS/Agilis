...
public enum NarrowPhase {
    ...
    public static func testAABBvsAABB(
        posA: Vector2, halfA: Vector2,
        posB: Vector2, halfB: Vector2
    ) -> Contact? {
    ...
    public static func testCircleVsCircle(
        posA: Vector2, radiusA: Float,
        posB: Vector2, radiusB: Float
    ) -> Contact? {
        let diff = posB - posA
    ...
    public static func testAABBvsCircle(
        ...
        let closestX = clamp(circlePos.x, min: aabbPos.x - halfExtents.x, max: aabbPos.x + halfExtents.x)
        let closestY = clamp(circlePos.y, min: aabbPos.y - halfExtents.y, max: aabbPos.y + halfExtents.y)
        ...
        if dist < PhysicsConstants.Tolerance.vectorLength {
            ...
            if dx < dy {
                ...
                let point = Vector2(x: aabbPos.x + halfExtents.x * sign, y: circlePos.y)
                return Contact(normal: normal, penetration: penetration, point: point)
            } else {
                let sign: Float = circlePos.y < aabbPos.y ? -1 : 1
                let normal = Vector2(x: 0, y: sign)
                let penetration = dy + radius
                let point = Vector2(x: circlePos.x, y: aabbPos.y + halfExtents.y * sign)
                return Contact(normal: normal, penetration: penetration, point: point)
            }
        }
    ...
    public static func testPolygonVsPolygon(
        verticesA: [Vector2], normalsA: [Vector2],
        verticesB: [Vector2], normalsB: [Vector2]
    ) -> Contact? {
        ...
        return Contact(normal: bestNormal, penetration: minOverlap, point: contactPoint)
    }
    ...
    public static func testPolygonVsCircle(
        vertices: [Vector2], normals: [Vector2],
        circlePos: Vector2, radius: Float
    ) -> Contact? {
        var minOverlap: Float = .infinity
        var bestNormal = Vector2.zero
    ...
    public static func testPolygonVsAABB(
        ...
        return testPolygonVsPolygon(
            verticesA: vertices, normalsA: normals,
            verticesB: aabbVerts, normalsB: aabbNorms
        )
    }
    ...
    public static func test(
        shapeA: CollisionShape, posA: Vector2, rotA: Float,
        shapeB: CollisionShape, posB: Vector2, rotB: Float
    ) -> Contact? {
        ...
        case (.aabb(let halfA), .aabb(let halfB)):
            if rotA == 0 && rotB == 0 {
                return testAABBvsAABB(posA: posA, halfA: halfA, posB: posB, halfB: halfB)
            }
            ...
            return testPolygonVsPolygon(verticesA: vertsA, normalsA: normsA,
                                        verticesB: vertsB, normalsB: normsB)

        // Circle vs Circle
        case (.circle(let rA), .circle(let rB)):
            return testCircleVsCircle(posA: posA, radiusA: rA, posB: posB, radiusB: rB)

        // AABB vs Circle
        case (.aabb(let half), .circle(let r)):
            if rotA == 0 {
                return testAABBvsCircle(aabbPos: posA, halfExtents: half,
                                        circlePos: posB, radius: r)
            }
            let verts = rotatedAABBVertices(pos: posA, half: half, rot: rotA)
            let norms = computeNormals(verts)
            return testPolygonVsCircle(vertices: verts, normals: norms,
                                       circlePos: posB, radius: r)

        // Circle vs AABB (swap and negate normal)
        case (.circle(let r), .aabb(let half)):
            if rotB == 0 {
                guard let c = testAABBvsCircle(aabbPos: posB, halfExtents: half,
                                               circlePos: posA, radius: r) else { return nil }
                return Contact(normal: -c.normal, penetration: c.penetration, point: c.point)
            }
            let verts = rotatedAABBVertices(pos: posB, half: half, rot: rotB)
            let norms = computeNormals(verts)
            guard let c = testPolygonVsCircle(vertices: verts, normals: norms,
                                              circlePos: posA, radius: r) else { return nil }
            return Contact(normal: -c.normal, penetration: c.penetration, point: c.point)

        // Polygon vs Polygon
        case (.polygon(let polyA), .polygon(let polyB)):
            let vertsA = transformVertices(polyA.vertices, position: posA, rotation: rotA)
            let normsA = rotateNormals(polyA.normals, rotation: rotA)
            let vertsB = transformVertices(polyB.vertices, position: posB, rotation: rotB)
        ...
        case (.polygon(let poly), .circle(let r)):
            let verts = transformVertices(poly.vertices, position: posA, rotation: rotA)
            let norms = rotateNormals(poly.normals, rotation: rotA)
            return testPolygonVsCircle(vertices: verts, normals: norms,
                                       circlePos: posB, radius: r)

        // Circle vs Polygon (swap and negate)
        case (.circle(let r), .polygon(let poly)):
            let verts = transformVertices(poly.vertices, position: posB, rotation: rotB)
            let norms = rotateNormals(poly.normals, rotation: rotB)
            guard let c = testPolygonVsCircle(vertices: verts, normals: norms,
                                              circlePos: posA, radius: r) else { return nil }
            return Contact(normal: -c.normal, penetration: c.penetration, point: c.point)
        ...
        case (.polygon(let poly), .aabb(let half)):
            let polyVerts = transformVertices(poly.vertices, position: posA, rotation: rotA)
            let polyNorms = rotateNormals(poly.normals, rotation: rotA)
            if rotB == 0 {
                return testPolygonVsAABB(vertices: polyVerts, normals: polyNorms,
                                         aabbPos: posB, halfExtents: half)
            }
            let aabbVerts = rotatedAABBVertices(pos: posB, half: half, rot: rotB)
            let aabbNorms = computeNormals(aabbVerts)
            return testPolygonVsPolygon(verticesA: polyVerts, normalsA: polyNorms,
                                        verticesB: aabbVerts, normalsB: aabbNorms)

        // AABB vs Polygon (swap and negate)
        case (.aabb(let half), .polygon(let poly)):
            let polyVerts = transformVertices(poly.vertices, position: posB, rotation: rotB)
            let polyNorms = rotateNormals(poly.normals, rotation: rotB)
            if rotA == 0 {
                guard let c = testPolygonVsAABB(vertices: polyVerts, normals: polyNorms,
                                                aabbPos: posA, halfExtents: half) else { return nil }
                return Contact(normal: -c.normal, penetration: c.penetration, point: c.point)
            }
            let aabbVerts = rotatedAABBVertices(pos: posA, half: half, rot: rotA)
            let aabbNorms = computeNormals(aabbVerts)
            guard let c = testPolygonVsPolygon(verticesA: polyVerts, normalsA: polyNorms,
                                               verticesB: aabbVerts, normalsB: aabbNorms) else { return nil }
            return Contact(normal: -c.normal, penetration: c.penetration, point: c.point)
        }
    }
    ...
    private static func projectVertices(_ vertices: [Vector2], onto axis: Vector2) -> (min: Float, max: Float) {
    ...
    private static func deepestPoint(in vertices: [Vector2], along direction: Vector2) -> Vector2 {
    ...
    private static func aabbVertices(pos: Vector2, half: Vector2) -> [Vector2] {
        [
