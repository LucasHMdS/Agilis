...
internal enum JointSolver {
    ...
    static func preSolve(
        ...
        velocityA: Velocity2D, velocityB: Velocity2D,
        bodyA: RigidBody2D, bodyB: RigidBody2D,
        dt: Float
    ) {
        switch joint.definition {
        case .revolute(let def):
            preSolveRevolute(joint: &joint, def: def,
                             transformA: transformA, transformB: transformB,
                             bodyA: bodyA, bodyB: bodyB, dt: dt)
        case .distance(let def):
            preSolveDistance(joint: &joint, def: def,
                            transformA: transformA, transformB: transformB,
                            bodyA: bodyA, bodyB: bodyB, dt: dt)
        case .weld(let def):
            preSolveWeld(joint: &joint, def: def,
                         transformA: transformA, transformB: transformB,
                         bodyA: bodyA, bodyB: bodyB, dt: dt)
        case .prismatic(let def):
            preSolvePrismatic(joint: &joint, def: def,
                              transformA: transformA, transformB: transformB,
                              bodyA: bodyA, bodyB: bodyB, dt: dt)
        case .rope(let def):
            preSolveRope(joint: &joint, def: def,
                         transformA: transformA, transformB: transformB,
                         bodyA: bodyA, bodyB: bodyB, dt: dt)
        case .motor(let def):
            preSolveMotor(joint: &joint, def: def,
                          transformA: transformA, transformB: transformB,
                          bodyA: bodyA, bodyB: bodyB, dt: dt)
        }
    }
    ...
    static func warmStart(
        joint: inout Joint2D,
        velocityA: inout Velocity2D, velocityB: inout Velocity2D,
        bodyA: RigidBody2D, bodyB: RigidBody2D
    ) {
        ...
        case .revolute:
            ...
            let angularImpulse = joint.accumulatedMotorImpulse + joint.accumulatedLimitImpulse
        ...
        case .weld:
            ...
            velocityA.linear.x -= mA * p.x
            velocityA.linear.y -= mA * p.y
            velocityA.angular -= iA * (crossVV(joint.rA, p) + joint.accumulatedAngularImpulse)
            velocityB.linear.x += mB * p.x
            velocityB.linear.y += mB * p.y
            velocityB.angular += iB * (crossVV(joint.rB, p) + joint.accumulatedAngularImpulse)

        case .prismatic:
            warmStartPrismatic(joint: &joint,
    ...
    static func solveVelocity(
        ...
        switch joint.definition {
        case .revolute(let def):
            solveVelocityRevolute(joint: &joint, def: def,
                                 velocityA: &velocityA, velocityB: &velocityB,
                                 bodyA: bodyA, bodyB: bodyB)
        case .distance:
            solveVelocityDistance(joint: &joint,
                                 velocityA: &velocityA, velocityB: &velocityB,
                                 bodyA: bodyA, bodyB: bodyB)
        case .weld:
            solveVelocityWeld(joint: &joint,
                              velocityA: &velocityA, velocityB: &velocityB,
                              bodyA: bodyA, bodyB: bodyB)
        case .prismatic(let def):
            solveVelocityPrismatic(joint: &joint, def: def,
                                   velocityA: &velocityA, velocityB: &velocityB,
                                   bodyA: bodyA, bodyB: bodyB)
        case .rope(let def):
            solveVelocityRope(joint: &joint, def: def,
                              velocityA: &velocityA, velocityB: &velocityB,
                              bodyA: bodyA, bodyB: bodyB)
        case .motor(let def):
            solveVelocityMotor(joint: &joint, def: def,
                               velocityA: &velocityA, velocityB: &velocityB,
                               bodyA: bodyA, bodyB: bodyB)
        }
    }
    ...
    static func solvePosition(
        joint: inout Joint2D,
        transformA: inout Transform2D, transformB: inout Transform2D,
        bodyA: RigidBody2D, bodyB: RigidBody2D
    ) -> Float {
        switch joint.definition {
        case .revolute(let def):
            return solvePositionRevolute(joint: &joint, def: def,
                                        transformA: &transformA, transformB: &transformB,
                                        bodyA: bodyA, bodyB: bodyB)
        case .distance(let def):
            return solvePositionDistance(joint: &joint, def: def,
                                        transformA: &transformA, transformB: &transformB,
                                        bodyA: bodyA, bodyB: bodyB)
        case .weld:
            return solvePositionWeld(joint: &joint,
                                    transformA: &transformA, transformB: &transformB,
                                    bodyA: bodyA, bodyB: bodyB)
        case .prismatic(let def):
            return solvePositionPrismatic(joint: &joint, def: def,
                                          transformA: &transformA, transformB: &transformB,
                                          bodyA: bodyA, bodyB: bodyB)
        case .rope(let def):
            return solvePositionRope(joint: &joint, def: def,
                                     transformA: &transformA, transformB: &transformB,
                                     bodyA: bodyA, bodyB: bodyB)
        case .motor:
    ...
    private static func preSolveRevolute(
        joint: inout Joint2D, def: RevoluteJointDef,
        transformA: Transform2D, transformB: Transform2D,
        bodyA: RigidBody2D, bodyB: RigidBody2D,
        dt: Float
    ) {
        ...
        //     [-iA*rA.x*rA.y - iB*rB.x*rB.y,       mA+mB + iA*rA.x^2 + iB*rB.x^2]
        let k11 = mA + mB + iA * joint.rA.y * joint.rA.y + iB * joint.rB.y * joint.rB.y
        let k12 = -iA * joint.rA.x * joint.rA.y - iB * joint.rB.x * joint.rB.y
        let k22 = mA + mB + iA * joint.rA.x * joint.rA.x + iB * joint.rB.x * joint.rB.x
        joint.effectiveMass = invert2x2((k11, k12, k12, k22))

        // Effective angular mass for limits/motor
        joint.effectiveAngularMass = iA + iB > 0 ? 1.0 / (iA + iB) : 0
    ...
    private static func solveVelocityRevolute(
        ...
        if def.enableMotor && joint.limitState != .equal {
            ...
            let maxImpulse = def.maxMotorTorque // Already per-step scale handled by iteration
            joint.accumulatedMotorImpulse = clampFloat(oldImpulse + lambda, -maxImpulse, maxImpulse)
        ...
        let rhs = Vector2(x: -(cdot.x + joint.bias.x), y: -(cdot.y + joint.bias.y))
        ...
        joint.constraintForce = sqrtf(joint.accumulatedImpulse.x * joint.accumulatedImpulse.x
                                    + joint.accumulatedImpulse.y * joint.accumulatedImpulse.y)
        joint.constraintTorque = abs(joint.accumulatedLimitImpulse) + abs(joint.accumulatedMotorImpulse)
    ...
    private static func solvePositionRevolute(
        ...
        // Positional error
        let posError = (transformB.position + rB) - (transformA.position + rA)
        let errorLen = sqrtf(posError.x * posError.x + posError.y * posError.y)

        // Rebuild effective mass for current configuration
        let k11 = mA + mB + iA * rA.y * rA.y + iB * rB.y * rB.y
        let k12 = -iA * rA.x * rA.y - iB * rB.x * rB.y
        let k22 = mA + mB + iA * rA.x * rA.x + iB * rB.x * rB.x
        let invK = invert2x2((k11, k12, k12, k22))

        let correction = mul2x2(invK, Vector2(x: -posError.x, y: -posError.y))
    ...
    private static func preSolveDistance(
        joint: inout Joint2D, def: DistanceJointDef,
        transformA: Transform2D, transformB: Transform2D,
        bodyA: RigidBody2D, bodyB: RigidBody2D,
        dt: Float
    ) {
        ...
        if currentLength > linearSlop {
            joint.currentAxis = Vector2(x: u.x / currentLength, y: u.y / currentLength)
        } else {
        ...
        if def.frequencyHz > 0 && dt > 0 {
            ...
            joint.springGamma = joint.springGamma > 0 ? 1.0 / joint.springGamma : 0

            joint.springBias = (currentLength - restLength) * h * k * joint.springGamma

            let effectiveMass = invEffMass + joint.springGamma
            joint.effectiveScalarMass = effectiveMass > 0 ? 1.0 / effectiveMass : 0
        } else {
            // Rigid mode
            joint.springGamma = 0
            joint.springBias = 0
            joint.effectiveScalarMass = invEffMass > 0 ? 1.0 / invEffMass : 0

            // Baumgarte bias for rigid constraint
            if dt > 0 {
                joint.springBias = (currentLength - restLength) * baumgarteBeta / dt
            }
        }
    }
    ...
    private static func solveVelocityDistance(
        ...
        let cdot = (vpB.x - vpA.x) * joint.currentAxis.x + (vpB.y - vpA.y) * joint.currentAxis.y

        let lambda: Float
        if joint.springGamma > 0 {
            // Spring mode
            lambda = -joint.effectiveScalarMass * (cdot + joint.springBias + joint.springGamma * joint.accumulatedScalarImpulse)
        } else {
            // Rigid mode
            lambda = -joint.effectiveScalarMass * (cdot + joint.springBias)
        }
        ...
        let p = Vector2(x: joint.currentAxis.x * lambda, y: joint.currentAxis.y * lambda)
        velocityA.linear.x -= mA * p.x
        velocityA.linear.y -= mA * p.y
        velocityA.angular -= iA * crossVV(joint.rA, p)
        velocityB.linear.x += mB * p.x
    ...
    private static func preSolveWeld(
        joint: inout Joint2D, def: WeldJointDef,
        transformA: Transform2D, transformB: Transform2D,
        bodyA: RigidBody2D, bodyB: RigidBody2D,
        dt: Float
    ) {
        ...
        if def.frequencyHz > 0 && dt > 0 {
            ...
            let angleError = transformB.rotation - transformA.rotation - joint.referenceAngle
            joint.angularBias = angleError * h * k * joint.springGamma

            joint.effectiveAngularMass = totalInvI + joint.springGamma
            joint.effectiveAngularMass = joint.effectiveAngularMass > 0 ? 1.0 / joint.effectiveAngularMass : 0
        } else {
    ...
    private static func solveVelocityWeld(
        ...
        if joint.springGamma > 0 {
            lambdaAngular = -joint.effectiveAngularMass * (cdotAngular + joint.angularBias
                            + joint.springGamma * joint.accumulatedAngularImpulse)
        } else {
            lambdaAngular = -joint.effectiveAngularMass * (cdotAngular + joint.angularBias)
        }
    ...
    private static func solvePositionWeld(
        ...
        // Positional error
        let posError = (transformB.position + rB) - (transformA.position + rA)
        let linearError = sqrtf(posError.x * posError.x + posError.y * posError.y)
    ...
    private static func preSolvePrismatic(
        joint: inout Joint2D, def: PrismaticJointDef,
        transformA: Transform2D, transformB: Transform2D,
        bodyA: RigidBody2D, bodyB: RigidBody2D,
        dt: Float
    ) {
        ...
        let worldAxis = rotateVector(joint.localAxisA, angle: transformA.rotation)
        let perp = Vector2(x: -worldAxis.y, y: worldAxis.x)
        joint.currentAxis = worldAxis

        // Displacement from anchor A to anchor B
        let d = (transformB.position + joint.rB) - (transformA.position + joint.rA)

        // Perpendicular constraint effective mass: dot(d, perp) = 0
        let s1 = crossVV(d + joint.rA, perp)
        let s2 = crossVV(joint.rB, perp)
        let invEffMassPerp = mA + mB + iA * s1 * s1 + iB * s2 * s2
        joint.effectiveScalarMass = invEffMassPerp > 0 ? 1.0 / invEffMassPerp : 0
        ...
        // Perpendicular bias
        let perpError = perp.x * d.x + perp.y * d.y // dot(d, perp)
        ...
        if def.enableLimit {
            let translation = worldAxis.x * d.x + worldAxis.y * d.y // dot(d, axis)
            if def.upperTranslation - def.lowerTranslation < 2.0 * linearSlop {
                joint.limitState = .equal
    ...
    private static func warmStartPrismatic(
        ...
        let pPerp = Vector2(x: perp.x * joint.accumulatedScalarImpulse,
                            y: perp.y * joint.accumulatedScalarImpulse)
        // Limit + motor impulse along axis
        let axisImpulse = joint.accumulatedLimitImpulse + joint.accumulatedMotorImpulse
        let pAxis = Vector2(x: worldAxis.x * axisImpulse, y: worldAxis.y * axisImpulse)

        let totalP = Vector2(x: pPerp.x + pAxis.x, y: pPerp.y + pAxis.y)

        velocityA.linear.x -= mA * totalP.x
        velocityA.linear.y -= mA * totalP.y
        velocityA.angular -= iA * (crossVV(joint.rA, totalP) + joint.accumulatedAngularImpulse)
        velocityB.linear.x += mB * totalP.x
        velocityB.linear.y += mB * totalP.y
        velocityB.angular += iB * (crossVV(joint.rB, totalP) + joint.accumulatedAngularImpulse)
    }
    ...
    private static func solveVelocityPrismatic(
        ...
        if def.enableMotor && joint.limitState != .equal {
            let cdotMotor = relVel.x * worldAxis.x + relVel.y * worldAxis.y - def.motorSpeed
            let crA = crossVV(joint.rA, worldAxis)
            let crB = crossVV(joint.rB, worldAxis)
            let invMassMotor = mA + mB + iA * crA * crA + iB * crB * crB
            let effMassMotor = invMassMotor > 0 ? 1.0 / invMassMotor : 0

            var lambdaMotor = -effMassMotor * cdotMotor
            let oldMotor = joint.accumulatedMotorImpulse
            joint.accumulatedMotorImpulse = clampFloat(oldMotor + lambdaMotor,
                                                        -def.maxMotorForce, def.maxMotorForce)
            lambdaMotor = joint.accumulatedMotorImpulse - oldMotor

            let pMotor = Vector2(x: worldAxis.x * lambdaMotor, y: worldAxis.y * lambdaMotor)
        ...
        if def.enableLimit && joint.limitState != .inactive {
            ...
            let relVel2 = Vector2(x: vpB2.x - vpA2.x, y: vpB2.y - vpA2.y)
            let cdotLimit = relVel2.x * worldAxis.x + relVel2.y * worldAxis.y

            let crA = crossVV(joint.rA, worldAxis)
            let crB = crossVV(joint.rB, worldAxis)
            let invMassLimit = mA + mB + iA * crA * crA + iB * crB * crB
            let effMassLimit = invMassLimit > 0 ? 1.0 / invMassLimit : 0
            ...
            let pLimit = Vector2(x: worldAxis.x * lambdaLimit, y: worldAxis.y * lambdaLimit)
        ...
        let cdotAngular = velocityB.angular - velocityA.angular
        let lambdaAngular = -joint.effectiveAngularMass * (cdotAngular + joint.angularBias)
        joint.accumulatedAngularImpulse += lambdaAngular
        ...
        let relVel3 = Vector2(x: vpB3.x - vpA3.x, y: vpB3.y - vpA3.y)
        let cdotPerp = relVel3.x * perp.x + relVel3.y * perp.y

        let lambdaPerp = -joint.effectiveScalarMass * (cdotPerp + joint.bias.x)
    ...
    private static func solvePositionPrismatic(
        ...
        let rA = rotateVector(joint.localAnchorA, angle: transformA.rotation)
        let rB = rotateVector(joint.localAnchorB, angle: transformB.rotation)
        let d = (transformB.position + rB) - (transformA.position + rA)
        ...
        if def.enableLimit {
            let translation = worldAxis.x * d.x + worldAxis.y * d.y
    ...
    private static func preSolveRope(
        joint: inout Joint2D, def: RopeJointDef,
        transformA: Transform2D, transformB: Transform2D,
        bodyA: RigidBody2D, bodyB: RigidBody2D,
        dt: Float
    ) {
        ...
        // Baumgarte bias
        if dt > 0 {
            joint.springBias = (currentLength - def.maxLength) * baumgarteBeta / dt
        }
    }
    ...
    private static func solveVelocityRope(
        ...
        let lambda = -joint.effectiveScalarMass * (cdot + joint.springBias)
        ...
        let p = Vector2(x: joint.currentAxis.x * actualLambda, y: joint.currentAxis.y * actualLambda)
        velocityA.linear.x -= mA * p.x
        velocityA.linear.y -= mA * p.y
        velocityA.angular -= iA * crossVV(joint.rA, p)
        velocityB.linear.x += mB * p.x
    ...
    private static func preSolveMotor(
        joint: inout Joint2D, def: MotorJointDef,
        transformA: Transform2D, transformB: Transform2D,
        bodyA: RigidBody2D, bodyB: RigidBody2D,
        dt: Float
    ) {
        ...
        let targetPos = transformA.position + rotateVector(def.linearOffset, angle: transformA.rotation)
        let linearError = transformB.position - targetPos

        // Angular error: rotB - rotA - angularOffset
        let angularError = transformB.rotation - transformA.rotation - def.angularOffset

        // Bias from correctionFactor (analogous to Baumgarte but user-tunable)
        if dt > 0 {
            joint.bias = linearError * (def.correctionFactor / dt)
            joint.angularBias = angularError * (def.correctionFactor / dt)
        }
    }
    ...
    private static func solveVelocityMotor(
        ...
        // Angular constraint
        let cdotAngular = velocityB.angular - velocityA.angular
        var lambdaAngular = -joint.effectiveAngularMass * (cdotAngular + joint.angularBias)

        // Clamp accumulated angular impulse magnitude to maxTorque
        let oldAngular = joint.accumulatedAngularImpulse
        joint.accumulatedAngularImpulse = clampFloat(oldAngular + lambdaAngular,
                                                      -def.maxTorque, def.maxTorque)
        ...
        let rhs = Vector2(x: -(cdotLinear.x + joint.bias.x), y: -(cdotLinear.y + joint.bias.y))
        ...
        let mag = sqrtf(joint.accumulatedImpulse.x * joint.accumulatedImpulse.x
                      + joint.accumulatedImpulse.y * joint.accumulatedImpulse.y)
...
/// | c d |              | -c  a |
@inline(__always)
internal func invert2x2(_ m: (Float, Float, Float, Float)) -> (Float, Float, Float, Float) {
    let det = m.0 * m.3 - m.1 * m.2
    guard abs(det) > PhysicsConstants.Tolerance.matrixDeterminant else { return (0, 0, 0, 0) }
    let invDet = 1.0 / det
...
@inline(__always)
internal func mul2x2(_ m: (Float, Float, Float, Float), _ v: Vector2) -> Vector2 {
    Vector2(x: m.0 * v.x + m.1 * v.y, y: m.2 * v.x + m.3 * v.y)
}

/// Clamp a float between min and max.
@inline(__always)
internal func clampFloat(_ value: Float, _ minVal: Float, _ maxVal: Float) -> Float {
