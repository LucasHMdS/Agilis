...
// ---- GLFW Key Code Constants (matching Agilis Key enum raw values) ----
...
struct PlatformWindow {
    ...
    // Keyboard
    bool keys[MAX_KEYS];
    bool keys_pressed[MAX_KEYS];           // sticky: set on DOWN, cleared per poll
    uint32_t char_pressed;

    // Mouse
    bool mouse_buttons[MAX_MOUSE_BUTTONS];
    bool mouse_buttons_pressed[MAX_MOUSE_BUTTONS]; // sticky: set on DOWN, cleared per poll
...
static int vk_to_agilis_key(WPARAM vk) {
    // Letters A-Z: VK_A(0x41)=65 matches AGILIS_KEY_A(65) directly
    if (vk >= 'A' && vk <= 'Z') return (int)vk;
    // Digits 0-9: VK_0(0x30)=48 matches AGILIS_KEY_0(48) directly
...
float platform_gamepad_axis(int index, int axis) {
    if (index < 0 || index >= MAX_GAMEPADS) return 0;
    XINPUT_STATE state;
