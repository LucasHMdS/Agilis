...
struct CCDTests {
    ...
    func fastCircleThroughThinWallCCD() {
        ...
        let (world, _) = runPhysics { world, _ in
            ...
            world.addComponent(Velocity2D(linear: Vector2(x: 12000, y: 0)), to: bullet)
            world.addComponent(RigidBody2D(mass: 1, restitution: 0, gravityScale: 0,
                                           bodyType: .dynamic, useCCD: true), to: bullet)
    ...
    func fastCircleThroughThinWallNoCCD() {
        ...
        let (world, _) = runPhysics { world, _ in
            ...
            world.addComponent(Velocity2D(linear: Vector2(x: 12000, y: 0)), to: bullet)
            world.addComponent(RigidBody2D(mass: 1, restitution: 0, gravityScale: 0,
                                           bodyType: .dynamic, useCCD: false), to: bullet)
            world.addComponent(Collider2D(shape: .circle(radius: 3)), to: bullet)
    ...
    func fastAABBThroughThinWall() {
        let (world, _) = runPhysics { world, _ in
            ...
            world.addComponent(Velocity2D(linear: Vector2(x: 12000, y: 0)), to: box)
            world.addComponent(RigidBody2D(mass: 1, restitution: 0, gravityScale: 0,
                                           bodyType: .dynamic, useCCD: true), to: box)
    ...
    func ccdBodyPassesThroughTrigger() {
        ...
        let (world, _) = runPhysics { world, _ in
            ...
            world.addComponent(Collider2D(shape: .aabb(halfExtents: Vector2(x: 10, y: 10)),
                                          isTrigger: true), to: trigger)
        }
    ...
    func ccdRespectsLayerMask() {
        ...
        let (world, _) = runPhysics { world, _ in
            ...
            world.addComponent(RigidBody2D(mass: 1, restitution: 0, gravityScale: 0,
                                           bodyType: .dynamic, useCCD: true), to: bullet)
            world.addComponent(Collider2D(shape: .circle(radius: 3), layer: 2, mask: 2), to: bullet)

            let wall = world.createEntity()
            world.setName("wall", for: wall)
            world.addComponent(Transform2D(position: Vector2(x: 100, y: 0)), to: wall)
            world.addComponent(Collider2D(shape: .aabb(halfExtents: Vector2(x: 2, y: 50)),
                                          layer: 4, mask: 4), to: wall)
        }

        let bullet = world.entity(named: "bullet")!
        let pos = world.getComponent(Transform2D.self, from: bullet)!
        // Mismatched layers: CCD should not clamp, bullet passes through
        #expect(pos.position.x > 102, "CCD should not clamp against non-colliding layers (x=\(pos.position.x))")
    }
    ...
    func ccdZeroDisplacement() {
        ...
        let (world, _) = runPhysics { world, _ in
            ...
            world.addComponent(RigidBody2D(mass: 1, gravityScale: 0,
                                           bodyType: .dynamic, useCCD: true), to: bullet)
            world.addComponent(Collider2D(shape: .circle(radius: 3)), to: bullet)
        ...
        #expect(abs(pos.position.x - 50) < 0.1, "Zero velocity CCD body should stay put (x=\(pos.position.x))")
    }
    ...
    func ccdSlowBodySkips() {
        ...
        #expect(abs(pos.position.x - 1.0) < 0.5, "Slow CCD body should integrate normally (x=\(pos.position.x))")
    }
    ...
    func twoCCDBodiesApproachWall() {
        let (world, _) = runPhysics { world, _ in
            ...
            world.addComponent(Velocity2D(linear: Vector2(x: 12000, y: 0)), to: b1)
            world.addComponent(RigidBody2D(mass: 1, restitution: 0, gravityScale: 0,
                                           bodyType: .dynamic, useCCD: true), to: b1)
            ...
            world.addComponent(Velocity2D(linear: Vector2(x: 18000, y: 0)), to: b2)
            world.addComponent(RigidBody2D(mass: 1, restitution: 0, gravityScale: 0,
                                           bodyType: .dynamic, useCCD: true), to: b2)
    ...
    func fastFallingBodyHitsFloor() {
        ...
        let (world, _) = runPhysics(gravity: Vector2(x: 0, y: 50000)) { world, _ in
            ...
            world.addComponent(Velocity2D(linear: Vector2(x: 0, y: 30000)), to: ball)
            world.addComponent(RigidBody2D(mass: 1, restitution: 0, gravityScale: 1,
                                           bodyType: .dynamic, useCCD: true), to: ball)
    ...
    func translatingRotatingRodDetectsWall() {
        ...
        let (world, _) = runPhysics { world, _ in
            ...
            world.addComponent(PreviousTransform2D(position: Vector2(x: 0, y: 0), rotation: 0), to: rod)
            world.addComponent(Velocity2D(linear: Vector2(x: 12000, y: 0), angular: 30), to: rod)
    ...
    func angularDisplacementEntersCCDSweep() {
        ...
        let (world, _) = runPhysics { world, _ in
            ...
            world.addComponent(Transform2D(position: Vector2(x: 0, y: 0)), to: rod)
            world.addComponent(PreviousTransform2D(position: Vector2(x: 0, y: 0), rotation: 0), to: rod)
            world.addComponent(Velocity2D(linear: Vector2(x: 200, y: 0), angular: 60), to: rod)
            world.addComponent(RigidBody2D(mass: 1, gravityScale: 0, bodyType: .dynamic, useCCD: true), to: rod)
            world.addComponent(Collider2D(shape: .aabb(halfExtents: Vector2(x: 50, y: 2))), to: rod)
        ...
        #expect(abs(transform.rotation) < fullAngularDisp - 0.1,
    ...
    func rotationClampedWithPosition() {
        ...
        let (world, _) = runPhysics { world, _ in
            ...
            world.setName("bullet", for: bullet)
            world.addComponent(Transform2D(position: Vector2(x: 0, y: 0)), to: bullet)
            world.addComponent(PreviousTransform2D(position: Vector2(x: 0, y: 0), rotation: 0), to: bullet)
            world.addComponent(Velocity2D(linear: Vector2(x: 12000, y: 0), angular: 30), to: bullet)
            world.addComponent(RigidBody2D(mass: 1, gravityScale: 0, bodyType: .dynamic, useCCD: true), to: bullet)
            world.addComponent(Collider2D(shape: .circle(radius: 3)), to: bullet)
        ...
        #expect(abs(transform.rotation) < fullAngularDisp,
                "Rotation should be clamped to TOI fraction (rot=\(transform.rotation))")
        #expect(abs(transform.rotation) > 0,
    ...
    func circleAngularVelocityExact() {
        ...
        let (world, _) = runPhysics { world, _ in
            ...
            world.addComponent(Transform2D(position: Vector2(x: 0, y: 0)), to: bullet)
            world.addComponent(PreviousTransform2D(position: Vector2(x: 0, y: 0), rotation: 0), to: bullet)
            world.addComponent(Velocity2D(linear: Vector2(x: 12000, y: 0), angular: 100), to: bullet)
            world.addComponent(RigidBody2D(mass: 1, gravityScale: 0, bodyType: .dynamic, useCCD: true), to: bullet)
            world.addComponent(Collider2D(shape: .circle(radius: 3)), to: bullet)
    ...
    func bilateralCCDBulletsBlocked() {
        ...
        let (world, _) = runPhysics { world, _ in
            ...
            world.addComponent(Velocity2D(linear: Vector2(x: 12000, y: 0)), to: bulletA)
            world.addComponent(RigidBody2D(mass: 1, restitution: 0, gravityScale: 0,
                                           bodyType: .dynamic, useCCD: true), to: bulletA)
            ...
            world.addComponent(Velocity2D(linear: Vector2(x: -12000, y: 0)), to: bulletB)
            world.addComponent(RigidBody2D(mass: 1, restitution: 0, gravityScale: 0,
                                           bodyType: .dynamic, useCCD: true), to: bulletB)
        ...
        #expect(posA.position.x < posB.position.x,
    ...
    func ccdVsNonCCDExistingBehavior() {
        ...
        let (world, _) = runPhysics { world, _ in
            ...
            world.addComponent(RigidBody2D(mass: 1, gravityScale: 0, bodyType: .dynamic, useCCD: false), to: target)
            world.addComponent(Collider2D(shape: .circle(radius: 5)), to: target)
        }
    ...
    func bilateralCCDWithAngularSweep() {
        ...
        let (world, _) = runPhysics { world, _ in
            ...
            world.addComponent(PreviousTransform2D(position: Vector2(x: 0, y: 0), rotation: 0), to: rodA)
            world.addComponent(Velocity2D(linear: Vector2(x: 12000, y: 0), angular: 10), to: rodA)
            world.addComponent(RigidBody2D(mass: 1, gravityScale: 0, bodyType: .dynamic, useCCD: true), to: rodA)
            ...
            world.addComponent(PreviousTransform2D(position: Vector2(x: 200, y: 0), rotation: 0), to: rodB)
            world.addComponent(Velocity2D(linear: Vector2(x: -12000, y: 0), angular: -10), to: rodB)
            world.addComponent(RigidBody2D(mass: 1, gravityScale: 0, bodyType: .dynamic, useCCD: true), to: rodB)
